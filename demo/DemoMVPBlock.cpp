...
int main (
  ...
  DataTopology::DistributeVectorBlock( x_local, x_global, proc_root,
                                       mpi_comm_rows, mpi_comm_columns );
  ...
  // -- compute y := A * x
  Vector<double,int> y_local( A_local.GetNumbRows( ) );
  BlasMpi::MatrixVectorProductBlock( y_local, A_local, x_local,
  ...
  DataTopology::AssembleVectorBand( y_global, y_local,
                                    proc_root, mpi_comm_columns );
  ...
  // -- write to csv
  if ( proc_numb == proc_root ) {
    y_global.WriteToFileCsv("mvp_block.csv");
  }
